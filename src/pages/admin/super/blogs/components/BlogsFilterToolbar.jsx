      ...
      <div className="flex items-center justify-between p-3 md:p-4">
        ...
        <div className="flex items-center space-x-2">
          <span className="text-xs sm:text-sm text-muted-foreground hidden sm:block">
            {filteredCount} blog post{filteredCount !== 1 ? 's' : ''} found
          </span>
          {hasActiveFilters && (
            <Button
              variant="ghost"
      ...
      <div className="hidden md:block p-4 space-y-4 border-t border-border">
        ...
        {activeFiltersCount > 0 && (
          <div className="flex flex-wrap gap-2 pt-3 border-t border-border">
            ...
            {categoryFilter !== 'all' && (
              <div className="flex items-center space-x-1 bg-primary/10 text-primary px-3 py-1 rounded-full text-xs">
                <span>Category: {categoryOptions.find(opt => opt.value === categoryFilter)?.label}</span>
                <button
      ...
      {!isExpanded && activeFiltersCount > 0 && (
        <div className="md:hidden p-3 border-t border-border">
          <div className="text-sm text-muted-foreground">
            ...
            {categoryFilter !== 'all' && (
              <span className="text-xs ml-2">• Category: {categoryOptions.find(opt => opt.value === categoryFilter)?.label}</span>
            )}
          </div>
        </div>
      )}
...
export default BlogsFilterToolbar;
