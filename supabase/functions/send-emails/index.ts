    ...
    switch (emailType) {
      ...
      case 'custom_quote_sent': // Support both for backward compatibility
        ...
        if (quoteTemplate.success) {
          // Build conditional sections based on language
          const isFrench = quoteLanguage === 'fr';
          const isDutch = quoteLanguage === 'nl';
          
          // VAT section HTML
          const vatEnabled = emailData.variables?.vat_enabled === 'true';
          ...
          // VAT section text
          const vatSectionText = vatEnabled
            ? `${isFrench ? 'TVA' : isDutch ? 'BTW' : 'VAT'} (${emailData.variables?.vat_percentage || '0%'}): ${emailData.variables?.vat_amount || '0€'}\n`
            : '';
